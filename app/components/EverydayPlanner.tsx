...
export default function EverydayPlanner({ context }: { context: Context }) {
  ...
  return <section className="everyday-planner wellness-toolkit" id="everyday-plan">
    ...
    <div className="wellness-tools">
      <section className="wellness-tool planner-food-entry" role="group" aria-labelledby="planner-food-heading"><div className="planner-card-title"><span>01</span><div><h4 id="planner-food-heading">Your real foods and meals</h4><small>OPTIONAL</small></div></div><label htmlFor="usual-foods">Type specific foods, broad patterns, or cultural and family meals</label><textarea id="usual-foods" maxLength={280} value={foodText} onChange={e=>{setFoodText(e.target.value);change();}} placeholder="Example: fried chicken with rice and beans, jollof rice, cereal, salmon, pizza, or a family recipe"/><small>{foodText.length}/280 characters</small><p>Use your own words. The planner looks for familiar food signals, keeps unrecognized meal names, and explains what it heard.</p><div className="planner-quick-picks"><b>Or tap a few foods you enjoy</b><div className="wellness-chips">{foodChoices.map(f=><button type="button" key={f} aria-pressed={likes.includes(f)} className={likes.includes(f)?"chosen":""} onClick={()=>toggle(f,likes,setLikes)}>{f}</button>)}</div><p>These quick choices work with anything you type above.</p></div></section>
      <section className="wellness-tool" role="group" aria-labelledby="planner-routine-heading"><div className="planner-card-title"><span>02</span><div><h4 id="planner-routine-heading">Make it fit your routine</h4><small>OPTIONAL</small></div></div><label>Eating preference<select value={diet} onChange={e=>{setDiet(e.target.value);change();}}><option value="any">No preference / skip</option><option value="vegetarian">Vegetarian</option><option value="vegan">Vegan</option></select></label><label className="planner-check"><input type="checkbox" checked={quick} onChange={e=>{setQuick(e.target.checked);change();}}/>Minimal preparation</label><label className="planner-check"><input type="checkbox" checked={budget} onChange={e=>{setBudget(e.target.checked);change();}}/>Everyday budget options</label></section>
      <section className="wellness-tool" role="group" aria-labelledby="planner-avoid-heading"><div className="planner-card-title"><span>03</span><div><h4 id="planner-avoid-heading">Foods to avoid</h4><small>OPTIONAL</small></div></div><div className="wellness-chips">{avoidChoices.map(a=><button type="button" key={a} aria-pressed={avoids.includes(a)} className={avoids.includes(a)?"chosen":""} onClick={()=>toggle(a,avoids,setAvoids)}>{a}</button>)}</div><label className="planner-check"><input type="checkbox" checked={otherRestriction} onChange={e=>{setOtherRestriction(e.target.checked);change();}}/>Another allergy or a medically prescribed eating plan</label><p>This list cannot check every allergy or cross-contact risk. Check ingredients; select “another allergy” for a checklist without food suggestions.</p></section>
      <section className="wellness-tool" role="group" aria-labelledby="planner-visit-heading"><div className="planner-card-title"><span>04</span><div><h4 id="planner-visit-heading">Prepare for your next visit</h4><small>OPTIONAL</small></div></div><label>Do you take a vitamin D supplement?<select value={supplement} onChange={e=>{setSupplement(e.target.value);change();}}><option value="skip">Not sure / skip</option><option value="yes">Yes</option><option value="no">No</option></select></label><label>Do you have a previous vitamin D test result?<select value={lab} onChange={e=>{setLab(e.target.value);change();}}><option value="skip">Not sure / skip</option><option value="yes">Yes</option><option value="no">No</option></select></label></section>
    </div>
